...
import { renderSocRing } from '../../../helpers/render-soc-ring';
					...
					${
						batteryCount === 1
							? renderSocRing(
									...
									(config.battery?.invert_flow
										? (data.batteryPower ?? 0) > 0
										: (data.batteryPower ?? 0) < 0) && !data.isFloating,
								)
							: ''
					}
					${createTextWithPopup(
						...
						config.wide && batteryCount === 2
							? renderSocRing(
									...
									(config.battery2?.invert_flow
										? (data.battery2Power ?? 0) > 0
										: (data.battery2Power ?? 0) < 0) && !data.isFloating2,
								)
							: ''
					}
					${createTextWithPopup(
