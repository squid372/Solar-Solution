...
export function renderPVFlow(
	...
	// Hot-core + pulse-wave line overlays (cloned from the flow path), only
	// when glow is on and power is flowing.
	...
	const lineOverlays =
		showDot && glow
			? svg`
				...
				<use href="#${lineId}" xlink:href="#${lineId}" class="${pulseClass}" />`
			: svg``;
