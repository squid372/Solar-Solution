...
function buildFlows() {
  return svg`
		${FLOWS.map((f) => renderPVFlow(f.id, f.d, f.color, 3, 1000, f.dur, false, 1))}
...
const mono = panel('Mono theme (desaturated)', true, {
  theme: 'mono',
  activity: 0.7,
  charging: true,
});
const lite = panel('Effects-lite (intensity 1)', true, {
  theme: 'neon',
  activity: 0.7,
  charging: true,
  intensity: 1,
});
const reduced = panel('Reduced motion', true, {
  theme: 'neon',
  activity: 0.7,
  charging: true,
  reducedMotion: true,
